...
function base_url($slug = '') {
    return str_replace('install/','', getRoot().$slug);
}
...
<html>
<head>
    <title>TCrypto - Installation</title>
...
<div class="container">
    <?php if ($step == 1):?>
        <h4>TCrypto Installation</h4>
        <p>This wizard will work you through TCrypto installation process , to continue press the continue button</p>
        <p>
    ...
    <?php elseif($step == 3):?>
        <form action="<?php echo url('?step=3')?>" method="POST">
            ...
            <div class="form-group">
                <label>Openexchangerates.org APP ID</label>
                <input required class="form-control" name="app_id" value="<?php echo input('app_id')?>" type="text"/>
                <span>Fiat currencies rates are pulled from openexchangerates.org. Please sign up and get a free API key (App ID),
                <a href="" onclick="window.open('https://openexchangerates.org/signup/free')">Signup here</a></span>
            </div>
    ...
    <?php elseif($step == 4):?>
        <div class="alert alert-success">Congratulations!!, TCrypto has been installed successfully on your server</div>

        <h6>Setup <strong>Cron Job</strong></h6>
        <hr/>
        <p>
            TCrypto needs to periodically update cryptocurrency market data from various internet sources.
